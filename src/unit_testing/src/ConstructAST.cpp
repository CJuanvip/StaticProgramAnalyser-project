...
// }
  ...
  Var nU{"u"}; Expr u = nU;
  StmtLst ifThen;
  ASTBuilder(ifThen).Assign("x", one);
  StmtLst ifElse;
  ASTBuilder(ifElse).Assign("y", one);
  StmtLst whileBody;
  ASTBuilder(whileBody).Assign("t", one);
  StmtLst stmtsF;

  Eq eqExpression1;
  eqExpression1.left = std::shared_ptr<Expr>(new Expr); *eqExpression1.left = one;
  eqExpression1.right = std::shared_ptr<Expr>(new Expr); *eqExpression1.right = one;
  CondExpr ifCond;
  ifCond = eqExpression1;

  Eq eqExpression2;
  eqExpression2.left = std::shared_ptr<Expr>(new Expr); *eqExpression2.left = one;
  eqExpression2.right = std::shared_ptr<Expr>(new Expr); *eqExpression2.right = one;
  CondExpr whileCond;
  whileCond = eqExpression2;
  ASTBuilder(stmtsF).Assign("z", one)
                    .If(ifCond, ifThen, ifElse)
                    .While(whileCond, whileBody);
  ProcDef procF("f", stmtsF); 

  StmtLst stmtsG;
  ASTBuilder(stmtsG).Assign("x", one)
        .Assign("y", one)
        .Assign("z", one)
        .Assign("t", one)
        .Call("f")
        ...
        .Assign("v", z)
        .Call("h");
  ProcDef procG("g", stmtsG);
  StmtLst stmtsH;
  ASTBuilder(stmtsH).Assign("u", t);
  ProcDef procH("h", stmtsH);
  StmtLst stmtsI;
  ASTBuilder(stmtsI).Call("h").Assign("u", t);
  ProcDef procI("i", stmtsI);
  Program prog{procF, procG, procH, procI};
  return prog;  
}